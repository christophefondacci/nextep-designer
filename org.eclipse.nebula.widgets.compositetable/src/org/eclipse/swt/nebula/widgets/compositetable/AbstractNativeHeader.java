...
/**
 * Class AbstractHeader. A Header class making it easier to implement a sorted table where clicking
 * on a header column sets or changes the sort order.
 * 
 * @author djo
 */
public abstract class AbstractNativeHeader extends Composite {
	...
	/**
	 * A Header object for CompositeTable that can tell clients to re-sort, and can move/resize the
	 * columns (if used with an appropriate layout manager).
	 * 
	...
	/**
	 * Clients must override this method to reset the current sort column/order if they want to
	 * support sorting.
	 * 
	...
	/**
	 * Clients must call this method (or its overloaded cousin) exactly once in their constructor to
	 * set the column text strings. All alignments will be set to SWT.LEFT.
	 * 
	...
	/**
	 * Clients must call this method (or its overloaded cousin) exactly once in their constructor to
	 * set the column text strings and alignments.
	 * 
	 * @param columnTextStrings String[] The text strings to display in each column
	 * @param alignments An array of SWT style bits. Each element is one of: SWT.LEFT, SWT.CENTER,
	 *        or SWT.RIGHT.
	 */
	...
	/**
	 * Method getWeights. If isFittingHorizontally, returns an array representing the percentage of
	 * the total width each column is allocated or null if no weights have been specified.
	 * <p>
	 * If !isFittingHorizontally, returns an array where each element is the minimum width in pixels
	...
	/**
	 * Method setWeights. If isFittingHorizontally, specifies an array representing the percentage
	 * of the total width each column is allocated or null if no weights have been specified.
	 * <p>
	 * If !isFittingHorizontally, specifies an array where each element is the minimum width in
	 * pixels of the corresponding column.
	 * <p>
	 * This property is ignored if the programmer has set a layout manager on the header and/or the
	 * row prototype objects.
	 * <p>
	 * The number of elements in the array must match the number of columns and if
	 * isFittingHorizontally, the sum of all elements must equal 100. If either of these constraints
	 * is not true, this property will be ignored and all columns will be created equal in width.
	 * 
	...
	/**
	 * Method isFittingHorizontally. Returns if the CompositeTable control will scale the widths of
	 * all columns so that they all fit into the available space. The default value is false.
	 * 
	 * @return Returns true if the table's actual width is set to equal the visible width; false
	 *         otherwise.
	 */
	...
	/**
	 * Method setFittingHorizontally. Sets if the CompositeTable control will scale the widths of
	 * all columns so that they all fit into the available space. The default value is false.
	 * 
