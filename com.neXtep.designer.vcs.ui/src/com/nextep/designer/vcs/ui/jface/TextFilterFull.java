...
public class TextFilterFull extends ViewerFilter implements ModifyListener {

	private Text text;
	private Viewer viewer;

	private TextFilterFull(Text text, Viewer viewer) {
		this.text = text;
		this.viewer = viewer;
	}
		...
		if (element instanceof IComparisonItem) {
			...
			for (ITypedObject o : ((ITypedNode) element).getChildren()) {
				...
				if (o instanceof IReferenceContainer) {
					for (IReferenceable r : ((IReferenceContainer) o).getReferenceMap().values()) {
						if (r instanceof INamedObject) {
							if (match(((INamedObject) r).getName(), pattern)) {
								return true;
							}
						}
					}
			...
			if (named != null && !text.isDisposed() && !"".equals(text.getText().trim())) { //$NON-NLS-1$
				if (named.getName().toUpperCase().contains(text.getText().toUpperCase())) {
					return true;
				}
			}
			if (element instanceof IReferenceContainer) {
				for (IReferenceable r : ((IReferenceContainer) element).getReferenceMap().values()) {
					if (r instanceof INamedObject) {
						if (match(((INamedObject) r).getName(), getPattern())) {
							return true;
						}
					}
				}
