 ...
 */
public class UndoCheckoutAction extends Action implements IModelOriented<IVersionable<?>>,
		IEventListener {
	...
	public void run() {
		...
		versioningService.undoCheckOut(null, getModel());
	}
	...
	private void handleEnablement() {
		if (model != null) {
			setEnabled(model.getVersion().getStatus() == IVersionStatus.CHECKED_OUT);
		} else {
			setEnabled(false);
		}
	}
