...
public class PostGreCheckConstraintGenerator extends SQLGenerator {
	...
	public IGenerationResult doDrop(Object model) {
		...
		s.appendSQL("\\echo Dropping check constraint '" + c.getName() + "' on table '"
				+ c.getConstrainedTable().getName() + "'..." + NEWLINE);
		s.appendSQL("ALTER TABLE " + c.getConstrainedTable().getName() + " DROP CONSTRAINT "
				+ c.getName()  + ";" + NEWLINE);
	...
	public IGenerationResult generateFullSQL(Object model) {
		final ICheckConstraint c = (ICheckConstraint) model;
		ISQLScript s = new SQLScript(c.getName(), "", "", ScriptType.CHECKCONS);
		s.appendSQL("\\echo Creating check constraint '" + c.getName() + "' on table '"
				+ c.getConstrainedTable().getName() + "'..." + NEWLINE);
		s.appendSQL("ALTER TABLE " + c.getConstrainedTable().getName() + " ADD CONSTRAINT "
				+ c.getName() + " CHECK ");
		s.appendSQL(c.getCondition()  + ";" + NEWLINE);
