 ...
 *******************************************************************************/
package com.nextep.datadesigner.dbgm.gui.jface;
 ...
 *
 */
public class ViewTriggableContentProvider implements IStructuredContentProvider {
	...
	public Object[] getElements(Object inputElement) {
		if(inputElement instanceof IVersionContainer) {
			...
			for(IVersionable<?> v : c.getContents()) {
				if(v instanceof ITriggable) {
					elts.add(v);
				}
				for(IReferenceable r : v.getReferenceMap().values()) {
					if(r instanceof ITriggable) {
						elts.add((ITriggable)r);
					}
				}
			}
			return elts.toArray();
