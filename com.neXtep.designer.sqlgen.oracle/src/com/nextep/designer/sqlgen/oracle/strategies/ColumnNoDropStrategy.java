...
import com.nextep.designer.sqlgen.oracle.generator.OracleColumnGenerator;
	...
	public IGenerationResult generateDrop(ISQLGenerator generator, Object modelToDrop) {
		...
		if (c.isNotNull()) {
			ISQLScript s = new SQLScript(c.getName(), "Column no drop strategy", "",
					ScriptType.DROP);
			s.appendSQL("Prompt Non-dropping column, removing NOT NULL condition" + NEWLINE);
			...
			r.addDropScript(
					new DatabaseReference(c.getType(), OracleColumnGenerator.getDbRefName(c)), s);
			...
			ISQLScript s = new SQLScript(c.getName(), "Column no drop strategy", "",
					ScriptType.DROP);
			s.appendSQL("Prompt Column drop strategy says we want to keep column "
					+ OracleColumnGenerator.getDbRefName(c) + "!" + NEWLINE);
			IGenerationResult r = GenerationFactory.createGenerationResult();
			r.addDropScript(
					new DatabaseReference(c.getType(), OracleColumnGenerator.getDbRefName(c)), s);
			return r;
