 ...
 * @author Christophe
 */
public class OracleProcedureParser extends AbstractTypedSqlParser {

	private static final Log log = LogFactory.getLog(OracleProcedureParser.class);
	...
	public IParseData parse(IParseable parseable, String sql) {
		try {
			...
			log.warn("Errors while parsing procedure");
			return null;
		}
	}
	...
	private IParseData internaleParse(IParseable parseable, String contentsToParse)
		...
		final IProcedure p = (IProcedure) parseable;
		...
		/** A flag indicating a procedure delcaration has started */
		...
		while (!token.isEOF()) {
			if (token == PackageScanner.PROC_ENDSPEC_TOKEN) {
				...
				if (pendingEnd) {
					...
					currentProc = p; // new
					// LightProcedure(doc.get(pkgScanner.getTokenOffset(),pkgScanner.getTokenLength()),null);
					p.setName(doc.get(pkgScanner.getTokenOffset(), pkgScanner.getTokenLength()));
