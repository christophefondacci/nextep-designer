 ...
 */
public class PLDocAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {

	/** The range of the pldoc scanner used to add parameters tag */
	private static final int PLDOC_RANGE = 10;
	/** The partitioning that this strategy operates on. */
	private final String fPartitioning;
	/** The currently edited package */
	 ...
	 */
	public PLDocAutoIndentStrategy(String partitioning, Object pkg) {
		fPartitioning = partitioning;
		if (pkg instanceof IPackage) {
			this.pkg = (IPackage) pkg;
		}
	}
	 ...
	 */
	protected int findLastCar(IDocument document, int offset, int end) throws BadLocationException {
		while (offset < end) {
	...
	private void indentAfterNewLine(IDocument d, DocumentCommand c) {
		...
		try {
			...
			if (firstNonWS < offset) {
				if (d.getChar(firstNonWS) == '/' && d.getLength() > firstNonWS + 1
						&& d.getChar(firstNonWS + 1) == '*') {
					// javadoc started on this line
					buf.append(" * "); //$NON-NLS-1$

					if (isPreferenceTrue(PreferenceConstants.EDITOR_CLOSE_PLDOCS)
						...
						if (isPreferenceTrue(PreferenceConstants.EDITOR_ADD_PLDOC_TAGS)) {
							...
							if (pkg != null) {
								pkg.setBodySourceCode(d.get());
								DBGMHelper.parse(pkg);
								TextPosition closestPos = null;
								IProcedure closestProc = null;
								for (IProcedure proc : pkg.getProcedures()) {
									TextPosition pos = pkg.getParseData().getPosition(proc);
									if (pos != null && pos.offset > c.offset) {
										if ((closestPos == null || pos.offset < closestPos.offset)
												&& pos.offset <= c.offset + PLDOC_RANGE) {
											closestPos = pos;
											closestProc = proc;
										}
									}
								...
								if (closestProc != null) {
									for (IProcedureParameter param : closestProc.getParameters()) {
										buf.append(lineDelimiter + indentation + " * @param "
												+ param.getName());
									}
								}
							}
					...
					// If we don't have a new comment and we are not in a comment partition,
					// we should not do anything.
					ITypedRegion partition = TextUtilities.getPartition(d, fPartitioning, p, false);
					if (partition.getType() != SQLPartitionScanner.COMMENT) {
						return;
					}
				}
			}
			...
			c.text = buf.toString().equals(c.text + "\t") ? c.text : buf.toString();
	...
	private boolean isPreferenceTrue(String preference) {
		return Activator.getDefault().getPreferenceStore().getBoolean(preference);
	}

	/**
	 * Returns the range of the Javadoc prefix on the given line in <code>document</code>. The
	 * prefix greedily matches the following regex pattern: <code>\w*\*\w*</code>, that is, any
	 * number of whitespace characters, followed by an asterix ('*'), followed by any number of
	...
	// * @throws BadLocationException if accessing the document fails
	// */
	// private String createJavaDocTags(IDocument document, DocumentCommand command, String
	// indentation, String lineDelimiter, ICompilationUnit unit)
	...
	// */
	// private String prepareTemplateComment(String comment, String indentation, String
	// lineDelimiter) {
	...
	// }
	//
	// private String createTypeTags(IDocument document, DocumentCommand command, String
	// indentation, String lineDelimiter, IType type)
	// throws CoreException, BadLocationException
	// {
	...
	// return null;
	// return prepareTemplateComment(comment.trim(), indentation, type.getJavaProject(),
	// lineDelimiter);
	// }
	// return null;
	// }
	//
	// private String createMethodTags(IDocument document, DocumentCommand command, String
	// indentation, String lineDelimiter, IMethod method)
	// throws CoreException, BadLocationException
	// {
	...
	// * <p>
	// * see also https://bugs.eclipse.org/bugs/show_bug.cgi?id=55325 (don't add parameters if the
	// member already has a comment)
	// * </p>
	// */
	...
	// return document.get(srcOffset, srcLength).lastIndexOf(token, nameRelativeOffset) ==
	...
	private boolean isNewComment(IDocument document, int commandOffset) {
		...
		try {
			...
			ITypedRegion partition = TextUtilities.getPartition(document, fPartitioning,
					commandOffset, false);
			if (partition.getType() != SQLPartitionScanner.COMMENT && commandOffset > 1) {
				partition = TextUtilities.getPartition(document, fPartitioning, commandOffset - 1,
						false);
